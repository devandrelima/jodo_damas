...
public class EsquerdaNaturalPecaRainha {
    public Coordenada[] buscarCoordenadaJogadorDeCima(Peca pecaAtual, Jogo tabuleiro, int buscador, Coordenada[] coordenadas, boolean inimigo) {
        ...
        if (proxLinha > 7 || proxColuna < 0) { // Não sai do tabuleiro, para quando chega no limite
    ...
    public Coordenada[] buscarCoordenadaJogadorDeBaixo(Peca pecaAtual, Jogo tabuleiro, int buscador, Coordenada[] coordenadas, boolean inimigo) {
        //Coordenada[] coordenadas = new Coordenada[5];
        //int contador = 0;
        ...
        if (proxPeca == null) { // Não tem peça na próxima coordenada
            coordenadas[buscador] = new Coordenada(proxLinha, proxColuna);
            buscador++;
            return buscarCoordenadaJogadorDeBaixo(new Peca(false, pecaAtual.getId(), new Coordenada(proxLinha, proxColuna)), tabuleiro, buscador, coordenadas, false);
            ...
            return buscarCoordenadaJogadorDeBaixo(proxPeca, tabuleiro, buscador, coordenadas, true);
